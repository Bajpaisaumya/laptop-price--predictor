...
st.title('Lappytol-Laptop_Price_Predictor')
...
#     query = np.array([company,type,ram,os,weight,touchscreen,ips,ppi,fullhd,cpu,hdd,ssd,gpu])
    
    


#     query = query.reshape(1,-1)
#     # column_names = ['Company', 'TypeName', 'Ram', 'OpSys','Weight', 'Touchscreen', 'IPS', 'ppi','Full HD',
...
#     # query_df_encoded = pd.get_dummies(query_df, columns=categorical_columns, drop_first=True)


#     predicted_price = (int(np.exp(pipe.predict(query)[0]))) 

#     st.title("Predicted Price: ${:,.2f}".format(predicted_price))
...
if st.button('Predict Price'):
        try:
            ...
            query = np.array([company,type,ram,weight,touchscreen,fullhd,ips,ppi,cpu,hdd,ssd,gpu,os])
